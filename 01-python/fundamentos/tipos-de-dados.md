...
Tipo de dado define que espécie de valor existe em memória e quais operações podem ser feitas com ele.

## Para que serve

Evitar confusão entre texto, número, booleano, lista, dicionário e ausência de valor.

## Por que esse conceito existe

Tipos existem porque computadores precisam saber como interpretar bits: número para soma, texto para concatenação, booleano para decisão.
...
Cada literal cria um objeto com tipo próprio: `int`, `bool` e `str`.
...
O ZYRON pode usar `str` para comandos, `dict` para configurações, `bool` para flags e `list` para histórico de mensagens.
...
**Pergunta:** Como você explicaria `Tipos de dados` para uma pessoa iniciante?
...
Tipos de dados é um fundamento que ajuda a transformar intenção em código executável. Entender esse assunto reduz erros, melhora leitura e cria base para módulos mais avançados do ZYRON.
