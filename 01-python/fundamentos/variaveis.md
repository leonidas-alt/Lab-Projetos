...
Variável é um nome que aponta para um objeto guardado em memória. Ela não é uma caixa física; é uma referência para um valor.

## Para que serve

Guardar valores para reutilizar, calcular, comparar e passar informações entre partes do programa.

## Por que esse conceito existe

Surgiu da necessidade de dar nomes humanos para dados manipulados por programas. Sem variáveis, precisaríamos repetir valores manualmente.
...
Python cria o objeto string `"ZYRON"` e faz o nome `nome` apontar para esse objeto.
...
No ZYRON, uma variável `comando = "abrir spotify"` pode guardar a intenção textual antes de enviá-la ao roteador.
...
**Pergunta:** Como você explicaria `Variáveis` para uma pessoa iniciante?
...
Variáveis é um fundamento que ajuda a transformar intenção em código executável. Entender esse assunto reduz erros, melhora leitura e cria base para módulos mais avançados do ZYRON.
